...
4. **Historians**: Databases that store operational data for later analysis. This historical data helps identify patterns, troubleshoot problems, and—in incident response scenarios —reconstruct what an attacker did.
...
In early 2024, the first ICS/OT (Industrial Control Systems / Operational Technology) malware, FrostyGoop, was discovered. The malware can directly interface with industrial control systems via the Modbus TCP protocol, enabling arbitrary reads and writes to device registers over TCP port 502.
